...
| 10 | `74c232e` | DO NOT SHIP (1) | **The lexer passed** — traced adversarially against nested block comments, multi-hash raw strings, comment markers inside strings, `'\''`, `'\\'`, lifetimes, byte chars and string/comment inversions, no defect. The blocker was that `top_level_functions` skipped indented lines, so a consumer in an `impl` or nested `mod` was invisible to the rule that mattered — **ordinary organization, not evasion** — and its own self-test documented that gap. Fixed by slice 3.14 (`4ed8282`) |
| 11 | `149790a` | **SHIP** | No blocking finding. The occurrence count closes the ordinary-organization hole: `RollbackParticipation` is pinned at five accounted occurrences, and because the type is module-private that count is **exhaustive over naming sites**. One Low — the lexer table's lifetime row could pass while a desync was live, since both markers sat outside the blanked span. Fixed by `22ec8eb`, verified RED by mutation (`left: 2, right: 3`) |
