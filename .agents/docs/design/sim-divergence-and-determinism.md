...
| Sphere-cast TOI noise vs large colliders: parry's GJK shape cast (parry3d-0.27.0 `gjk::minkowski_ray_cast`) takes a stagnation exit — when the upper TOI bound stops moving under float cancellation against a large shape's support coordinates, the last-chance return hands back the unrefined LOWER bound — so the sphere probe's `hit.distance` against the 1000 m ground slab came up one-sided up to ~172 mm SHORT — deterministic but pose-discontinuous. A standing divergence AMPLIFIER (mm-scale pose differences between the two ends → 10–40 kN per-wheel force differences through the 551 kN/m spring) and the at-rest limit-cycle pump (~2.2 kW measured, sustaining the ~12 mm / 0.29° hull wobble = the gunner-sight shake on flat ground) | one-sided distance error scales with collider extent: 0.25 mm at 5 m half-extent vs 139–172 mm at 500 m; 10–40 kN per-wheel force noise per tick at rest | **RETIRED — workaround deleted, defect now unreachable** — the fix, landed 2026-07-06, was witness-geometry reconstruction (`sphere_cast_ground_contact`, src/driving/contact.rs): the same hit's `point1`/`normal1` are exact even when the TOI is wrong (measured `point1.y = 0.000000` throughout), so the travel is recomputed from them — GUARDED (same-day hardening): the reconstruction is trusted only for non-penetrating casts with a finite witness, clamped to `[toi_based, toi_based + 0.20]` (0.20 m = the worst live-measured one-sided short error, 199.8 mm; parry's TOI is a lower bound, so the band caps any witness pathology at the old error scale); penetrating starts (parry swaps the witness for a penetration contact whose normal is unrelated to the cast axis) and non-finite witnesses fall back to the old conservative TOI path. `tests/spherecast_scale.rs` pinned the helper's math (reconstruction, band, fallbacks) and parry's TOI defect (the workaround-retirement tripwire); it did not bind the `apply_suspension` call site (a thin adapter over the helper) — that wiring's live guard was the idle at-rest harness metric (p.y spread ≲ 0.02 mm). Helper and test both went out wholesale at 9758d97 as phase-B collateral with the raycast-era suspension — the tripwire never fired. parry3d 0.27.0 still ships the defect, but live code has ZERO shape-cast call sites (every cast is a raycast or analytic), so it is unreachable; ADR-0015's ≤10 m tiling rule and `tests/shape_cast_reachability.rs` hold it for the first future consumer. Upstream report candidate #4: parry GJK shape-cast stagnation exit vs large shapes |
...
measurement: `upstream/avian-solver-constraint-order.md`. And bit-exactness IS on the
...
1. **Physics state is bit-exact on EVERY shared tick of both runs — the whole course, bump and
   ...
   `upstream/avian-solver-constraint-order.md`), with this instrument as the metric.
...
- **Parry workaround retirement was armed; it never fired.** Against the fork,
  `tests/spherecast_scale.rs` failed exactly as designed; at-rest idle with raw TOI + patched parry
  matched workaround-on (per-tick |dy| p99 0.027 vs 0.029 mm, no limit cycle); long course at
  baseline (94.85%, bit-exact physics). The reconstruction and its tripwire were instead deleted
  wholesale at 9758d97 as phase-B collateral, with the raycast-era suspension that called them —
  not on a parry release. No fork, no `[patch]`, no shape-cast call site remains; the defect is
  dormant behind ADR-0015's ≤10 m rule and `tests/shape_cast_reachability.rs`.
