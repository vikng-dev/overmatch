...
- **Steering is GRIPPIER than the first drive** (the strain law's fuller sub-saturation
  traction): turns 2–6× tighter, pivots ~2× faster, near no-slip radii. The settled element law
  is ISOTROPIC (full μN in every direction — no `lateral_ratio` dial); pivot authority / turn
  reluctance is tuned via `MU` (collateral: raises all grip together).
...
- **Turning costs forward speed.** Lateral scrub and longitudinal traction share one
  isotropic μN circle (no ellipse, no lateral share). MEASURED (T-34 lab): pivots are snappy
  (±144°/s, but that's the lab vehicle) while at-speed turns run wide — a 0.4-steer turn at
  5.4 m/s fits a ~53 m circle vs ~11 m no-slip. At-speed turn width is set by `MU` and footprint,
  not a lateral dial.
...
| Pivot reluctant / turns wide, or too darty | `MU` (sim const) | The law is isotropic — no lateral-share dial; `MU` sets all grip together |
...
| Bounce / contact loss in turns | `suspension.*` (ride freq / damping / `engage`) | Contact envelope derives support from these; no authored `support.*` fields exist |
