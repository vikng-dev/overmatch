...
- **UI strings rendered through Bevy `Text` must be printable ASCII (U+0020–U+007E).** Bevy's default font is an ASCII-only FiraMono subset with no fallback, so a non-ASCII glyph draws tofu on screen. Use `...` not `…`, `-` not `—`, `deg` not `°`. This applies ONLY to strings that reach `Text`; comments, logs (`info!`/`warn!`/`error!`/…), panics and asserts are unrestricted — the house style uses em dashes heavily there, do not touch them. `tests/ui_ascii.rs` guards the rendered set (it strips comments and diagnostic-macro arguments, then asserts the remaining string literals in the `Text`-spawning files are ASCII).
