...
The guarantee is positional only. Silhouette IS covered (silhouette error is surface deviation). Separately gated because deviation cannot see them: normal deviation, UV drift, defaulted tangents, degenerate faces, holes. The red-test class lives here — the shipped shoe's ONE defaulted-tangent vertex sits on a 50.16 mm edge that draws 16.2 budget pixels of wrong shading at L1's own 55.9 m switch, while passing every positional check AND the manifest's own `tangent_default_verts: 0` (which counts UV-degenerate faces, and this face is not one).
