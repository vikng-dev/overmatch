...
pub struct TankSpec {
    ...
    /// Pitch→X) and slew tuning; `on_tank_ready` resolves each name to its node and binds the servo.
...
mod tests {
    ...
    /// `tank::on_tank_ready`, but without launching Bevy: it reads the glTF node names directly and
    ...
    fn tiger_1_spec_binds_to_model() {
        ...
        // Fixed structural contract (mirrors `on_tank_ready`'s `REQUIRED` + the prefix scans).
        has("Hull");
        has("Center_Of_Mass");
        assert!(
